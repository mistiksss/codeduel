...
class Task(db.Model):
    ...
    def difficulty_label(self):
        """Russian label used in the task list and filters."""
        key = (self.difficulty or '').strip().lower()
        if key in DIFFICULTY_LABELS:
            return DIFFICULTY_LABELS[key]
        if key in DIFFICULTY_LABELS.values():
            return key
        return DIFFICULTY_LABELS.get('medium')
