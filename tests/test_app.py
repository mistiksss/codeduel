...
EXPECTED_ENDPOINTS = {
    ...
    'match.online_players': '/api/online_players',
...
class ModelSchemaTests(DatabaseTestCase):
    ...
    def test_task_columns(self):
        ...
        self.assertTrue(
            {'id', 'title', 'difficulty', 'points', 'time_limit', 'memory_limit'}.issubset(columns),
        )
...
class AuthAndPagesTests(DatabaseTestCase):
    ...
    def test_login_shows_error_on_bad_password(self):
    ...
    def test_register_rejects_duplicate_email(self):
        ...
        self.assertIn('уже зарегистрирован', body)
        self.assertEqual(User.query.filter_by(email='alice@example.com').count(), 1)

    def test_onboarding_saves_language(self):
        self.client.post(
            '/register',
            data={
                'username': 'nina',
        ...
        response = self.client.post('/onboarding', data={'language': 'python'})
        self.assertEqual(response.status_code, 302)
        self.assertIn('/matchmaking', response.headers['Location'])
        user = User.query.filter_by(username='nina').first()
...
class JudgingTests(DatabaseTestCase):
    ...
    def test_public_and_authed_pages_render(self):
        ...
        self.assertNotIn('i.postimg.cc', home)
        self.assertIn('fonts.css', home)
        self.assertNotIn('unpkg.com', home)
        self.assertNotIn('cdnjs.cloudflare.com', home)
        self.assertIn('vendor/three', home)
        self.assertIn('favicon', home)
        self.assertIn('+7 (905) 471 80 16', home)
        css_dir = Path(__file__).resolve().parents[1] / 'static'
        for css_name in (
            'main.css', 'main_reg.css', 'login.css', 'matchmaking.css', 'arena.css',
        ):
            css = (css_dir / css_name).read_text(encoding='utf-8')
            self.assertNotIn('Matrix-Code-PNG-Image.png', css)
            self.assertNotIn('matrix-bg.webp', css)
            self.assertNotIn('matrix-bg.jpg', css)
        fonts = (css_dir / 'fonts.css').read_text(encoding='utf-8')
        self.assertIn('radial-gradient', fonts)
        ...
        tasks_page = self.client.get('/tasks').get_data(as_text=True)
        self.assertIn('легкая', tasks_page)
        self.assertNotIn('task.difficulty_level', tasks_page)
        arena = self.client.get('/task/1').get_data(as_text=True)
        self.assertNotIn('unpkg.com', arena)
        self.assertNotIn('cdnjs.cloudflare.com', arena)
        self.assertNotIn('Код соперника', arena)
        self.assertNotIn('Вставка запрещена', arena)
        self.assertIn('vendor/monaco', arena)
        self.assertIn('Лимит памяти', arena)
        online = self.client.get('/api/online_players')
        self.assertEqual(online.status_code, 200)
        payload = online.get_json()
        self.assertIn('total_online', payload)
        self.assertIn('total_in_queue', payload)
        self.assertGreaterEqual(payload['total_online'], 1)
...
class MatchLifecycleTests(DatabaseTestCase):
    ...
    def test_cancel_search_does_not_delete_active_match(self):
        create_task(difficulty='easy')
        kate = create_user('kate', 'kate@example.com', elo=1000, is_online=True)
        leo = create_user('leo', 'leo@example.com', elo=1010, is_online=True)
        matchmaking = self.app.matchmaking_system
        matchmaking.find_opponent(kate.id, kate.elo, 'easy')
        second = matchmaking.find_opponent(leo.id, leo.elo, 'easy')
        match_id = second['match_id']
        login(self.client, 'leo@example.com')
        response = self.client.post('/api/matchmaking/cancel')
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.get_json().get('success'))
        self.assertIsNotNone(db.session.get(Match, match_id))
