...
from models import Match, MatchResult, MatchmakingQueue, User
...
@login_required
def play_with_bot():
    """Legacy training shortcut: send the player to the task list."""
...
def cancel_matchmaking():
    """Leave the queue and delete an unfinished match if one exists.

    Historical behaviour: cancelling search also removes the latest
    unfinished Match row for this user (and its MatchResult rows).
    """
    try:
